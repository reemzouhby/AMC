...
# ── 3. Same Split (random_state=42 guarantees identical test set) ─────────────
...
model = GoogLeNet1D(in_channels=2, n_classes=11).to(device)
model.load_state_dict(torch.load("googlenet1d_best.pth", weights_only=True))
model.eval()
print("GoogLeNet1D loaded! Running evaluation...")
...
plt.title("GoogLeNet1D — Confusion Matrix")
plt.tight_layout()
plt.savefig("googlenet1d_confusion_matrix.png", dpi=150)
plt.show()
